...
* **insert into xx values('s','s') 会导致SQL 没办法分类**

```
症状： 这种SQL语句，会严重干扰抽象语句的统计和排序，对于之后的slow 分析诸多不便。
解决方案： insert into xx values('s','s')  -> insert into xx values('s') 即可
```
...
以上的改动，均在我修改过的mysqlsla 版本中修复。预支详情，请看修改过的代码，都在注释中添加 --add by keithlan

[老的mysqlsla源码](https://github.com/Keithlan/Keithlan.github.io/blob/master/github_md/Mysql/SYSTEM_TOOLS/tz_slow/mysqlsla)

[修改后的mysqlsla源码](https://github.com/Keithlan/Keithlan.github.io/blob/master/github_md/Mysql/SYSTEM_TOOLS/tz_slow/mysqlsla_lc)
